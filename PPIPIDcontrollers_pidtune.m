...
% - Annotates settling time, overshoot, and and steady-state error
% - Prints gain, phase margin, overshoot, settling time, and steady-state error
%%----------------------------------------------------------------------
...
fprintf('P:   Kp=%.3f, PM=%.1f°, Ts=%.3f s, Mp=%.1f%%, ess=%.3f\n', CP.Kp, infoP.PhaseMargin, Ts_P,   Mp_P,   essP);
fprintf('PI:  Kp=%.3f, Ki=%.3f, PM=%.1f°, Ts=%.3f s, Mp=%.1f%%, ess=%.3f\n', CPI.Kp, CPI.Ki, infoPI.PhaseMargin, Ts_PI,  Mp_PI,  essPI);
fprintf('PID: Kp=%.3f, Ki=%.3f, Kd=%.3f, PM=%.1f°, Ts=%.3f s, Mp=%.1f%%, ess=%.3f\n', CPID.Kp, CPID.Ki, CPID.Kd, infoPID.PhaseMargin, Ts_PID, Mp_PID, essPID);
